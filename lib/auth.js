  ...
  callbacks: {
    async jwt({ token, user, trigger, session }) {
      if (user) {
        ...
        token.max_stores = user.max_stores;
      }
      if (trigger === 'update') {
        // Leer email_verified directo de DB (fuente de verdad)
        const users = await sql`SELECT email_verified, max_stores FROM users WHERE id = ${token.id}`;
        if (users.length > 0) {
          token.email_verified = users[0].email_verified;
          token.max_stores = users[0].max_stores;
        }
    ...
    async session({ session, token }) {
      ...
      session.user.max_stores    = token.max_stores;
      return session;
    },
  },
  pages: { signIn: '/login' },
