...
export function orderConfirmed({ username, orderId, storeName, items, total, pickupPointName }) {
  ...
  return {
    ...
    html: base(`
      ...
      ${pickupRow}
      <p class="note">Podés seguir el estado de tu pedido desde tu perfil en CnB.</p>
    `),
  };
}
...
export function orderStatusUpdate({ username, orderId, status, storeName }) {
  ...
  return {
    ...
    html: base(`
      ...
      <p>${cfg.msg}</p>
      <p class="note">Podés seguir el estado de tus pedidos desde tu perfil en CnB.</p>
    `),
  };
}
...
export function orderConfirmedBuyer({
  ...
  const rows = (items || []).map((i, idx) => `
    <tr style="background:${idx % 2 === 0 ? '#fff' : '#f9f8f7'};">
      <td style="padding:9px 12px;border-bottom:1px solid #f0ede8;font-size:0.8rem;">${i.name || i.product_name}</td>
...
export function orderNotificationStore({
  ...
  return {
    ...
    html: base(`
      ...
      <p class="note">Gestioná este pedido desde tu panel de administración en CnB.</p>
    `),
  };
}
