...
export default function WelcomePopupStore({ storeId, storeName, discountPercent, primaryColor }) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      const shown = localStorage.getItem('tnb_popup_store_' + storeId);
      if (shown === '1') return;
  ...
  const handleClose = () => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('tnb_popup_store_' + storeId, '1');
    }
    setIsVisible(false);
  };
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                color: '#fff',
                background: stripeColor,
                filter: isBtnHovered ? 'brightness(0.9)' : 'none',
                border: 'none',
                borderRadius: 6,
                cursor: 'pointer',
                marginBottom: 12,
